"""
Предсказывание трех цифр из MNIST. При необходимости можно сделать для большего кол-ва, но тогда надо
изменить вывод.
"""
...
""" Пробуем предсказать """
our_images = np.array([X_train[r.randrange(0, 60000)], X_train[r.randrange(0, 60000)], X_train[r.randrange(0, 60000)]])
from matplotlib import pyplot as plt

for one in our_images:
    plt.figure()
    plt.imshow(one.reshape((28,28)))
    #our_images = our_images.astype("float32")
    result = np.ndarray.flatten(model.predict(one.reshape(1, 784)))
    print('Prediction:')
