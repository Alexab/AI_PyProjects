"""
Эта программа основывается на "example1.py" из этого же проекта (просто слегка модифицирована для удобства измерений,
никаких дополнительных файлов для работы этого скрипта не требуется).
Здесь подставляются различные функции активации и ошибки. В конце файла есть таблица с результатами.

Программа добавляет значения точности в файл "data/ActivationsLosses.txt". После завершения программы полученный файл
нужно переименовать, чтобы при повторном запуске новые значения были записаны в новый файл.
